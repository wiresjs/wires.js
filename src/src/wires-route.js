(function() {
   ...
   domain.register("$router", ['$http', '$queryString', '$load', '$history'],
      function($http, $queryString, $load, $history) {
         return {
            ...
            loadStates: function(states) {
               ...
               domain.each(states, function(routeState, index) {
                  ...
                  self.stack = [];
                  console.log("Loaded in " + (new Date().getTime() - start), "ms")
               })
            },
            start: function() {
               var self = this;
